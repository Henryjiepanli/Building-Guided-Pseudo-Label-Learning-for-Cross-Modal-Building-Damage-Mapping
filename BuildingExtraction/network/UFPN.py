...
class SemanticFPNDecoder(nn.Module):
    def __init__(self,channel, feature_strides, num_classes):
        ...
        BN_relu = nn.Sequential(nn.SyncBatchNorm(self.channels,1e-5,0.1,True,True),nn.ReLU())
            ...
            for k in range(head_length):
                scale_head.append(
                    nn.Sequential(nn.Conv2d(
                        ...
                        self.channels,
                        kernel_size=3,
                        padding=1), nn.BatchNorm2d(self.channels,1e-5,0.1,True,True), nn.ReLU(inplace=True)))
...
class BasicConv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1):
        ...
        self.bn = nn.BatchNorm2d(out_planes,1e-5,0.1,True,True)
