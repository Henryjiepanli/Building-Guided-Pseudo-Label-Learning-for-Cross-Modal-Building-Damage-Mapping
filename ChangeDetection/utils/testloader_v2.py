...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        return {'A': img_A, 'B': img_B, 'building': building}
...
class Multi_Class_Segmentation_Dataset(data.Dataset):
    def __init__(self, img_A_root, img_B_root, building_root, trainsize):
    ...
    def __getitem__(self, index):
        ...
        image_A, image_B, building = self.load_img_and_mask(index)

        sample = {'A': image_A, 'B': image_B, 'building': building}
    ...
    def load_img_and_mask(self, index):
        ...
        building_mask = Image.open(self.buildings[index])
       
        return image_A, image_B, building_mask
    ...
    def filter_files(self):
        ...
        images_B = []
        buildings = []
        for img_A_path, img_B_path, building_path in zip(self.images_A, self.images_B, self.buildings):
...
def get_loader(img_A_root, img_B_root, building_root, trainsize, batchsize,  num_workers=4, shuffle=True, pin_memory=True):

    dataset = Multi_Class_Segmentation_Dataset(img_A_root = img_A_root, img_B_root = img_B_root, building_root = building_root, trainsize = trainsize)
